...
--     ваш email + пароль, включить Auto Confirm User.
--     ЭТОТ ПЕРВЫЙ ПОЛЬЗОВАТЕЛЬ АВТОМАТИЧЕСКИ СТАНЕТ АДМИНОМ --
--     никакого SQL для роли выполнять не нужно;
--  3) Authentication -> Sign In / Providers ->
--     Allow new users to sign up -> OFF;
--  4) три ключа проекта -> Vercel Environment Variables -> Redeploy.
--
-- Остальных сотрудников: создать в Supabase -> Authentication ->
-- Users, затем на странице «Сотрудники» в программе выдать роль и
-- объекты. Секретный ключ для этого НЕ нужен.
--
