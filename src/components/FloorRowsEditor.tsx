...
import { emptyStructureRow, type StructureRow } from "@/lib/buildings/generateUnits";

export function FloorRowsEditor({
  rows,
  onChange,
}: {
  rows: StructureRow[];
  ...
  const updateRow = <K extends keyof StructureRow>(
    index: number,
    key: K,
    value: StructureRow[K]
  ) => {
    onChange(rows.map((r, i) => (i === index ? { ...r, [key]: value } : r)));
  };

  const addRow = () => onChange([...rows, { ...emptyStructureRow }]);
          ...
          >
            <label className="flex flex-col gap-1 text-xs">
              <span className="font-medium text-slate-700">
                {t.buildings.floorBuilder.block}
              </span>
              <input
                value={row.block}
            ...
            <label className="flex flex-col gap-1 text-xs">
              <span className="font-medium text-slate-700">
                {t.buildings.floorBuilder.floor}
              </span>
              <input
                type="number"
                value={row.floor}
