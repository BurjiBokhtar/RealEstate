...
* **цена за 1 м² не доходит до квартир** — нужны триггеры
  `trg_apply_building_rate` (на `buildings`) и `trg_price_unit_from_building`
  (на `objects`) из блока 050. С ними цена расходится по квартирам сама:
  при смене ставки, при создании квартиры и при вводе площади. Кнопка
  «Применить» в карточке здания нужна только для квартир, созданных раньше.
...
* **график платежей не совпадает с суммой договора** — после смены суммы
  договора с рассрочкой график теперь пересчитывается сам (триггер из
  блока 055). Но это только для будущих правок; договоры, у которых
  расхождение накопилось раньше, нужно почистить один раз блоком **056** —
  он сам находит все такие договоры и пересчитывает у них неоплаченный
  остаток, не трогая уже принятые платежи. Без него у старых договоров
  печатный график продолжит показывать неверный остаток.
