...
--
-- Все миграции проекта, склеенные в правильном порядке. Для новой
-- базы достаточно выполнить ТОЛЬКО этот файл в Supabase SQL Editor:
-- GitHub -> этот файл -> Raw -> выделить всё -> скопировать ->
-- вставить -> Run.
--
-- После него остаётся:
--  1) Authentication -> Users -> создать первого пользователя;
--  2) выдать ему роль admin (SQL в supabase/README.md);
--  3) Authentication -> Sign In / Providers -> Allow new users
--     to sign up -> OFF.
--
